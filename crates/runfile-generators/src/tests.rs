...
use crate::vscode::*;
use crate::zed::*;
use runfile_parser::{CommandSpec, Runfile};
...
fn zed_target_with_args_gets_custom_args() {
	...
	let tasks = generate_zed_tasks(&runfile);
	assert_eq!(tasks[0].args, vec!["test", "$ZED_CUSTOM_ARGS"]);
...
fn zed_target_with_named_args_gets_custom_args() {
	...
	let tasks = generate_zed_tasks(&runfile);
	assert_eq!(tasks[0].args, vec!["deploy", "$ZED_CUSTOM_ARGS"]);
	assert_eq!(tasks[0].allow_concurrent_runs, Some(true));
}
...
fn jetbrains_generate_basic_target() {
	...
	assert!(configs[0].xml.contains("value=\"run build\""));
...
fn jetbrains_colon_in_target_name_becomes_capitalized_words() {
	...
	assert!(configs[0].xml.contains("value=\"run build:infrastructure\""));
}
...
fn jetbrains_check_recognizes_legacy_format() {
	// Files generated by older versions still carry the "Runfile / <target>" prefix; treat them as ours
...
fn jetbrains_target_with_multiple_colons() {
	...
	assert!(configs[0].xml.contains("run test:unit:fast"));
}
